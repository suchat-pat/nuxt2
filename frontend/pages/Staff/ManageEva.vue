<template>
    <v-container fluid class="py-10">
                <v-card>
                    <v-sheet class="text-center pa-6" color="">
                        <h1 class="text-h5 font-weight-bold text-maroon">จัดการผู้รับการประเมิน</h1>
                    </v-sheet>
                    <v-card-text>
                        ...
                        <v-table>
                            <thead>
                                <tr>
                                    <th class="text-center pa-2 border">ลำดับ</th>
                                    <th class="text-center pa-2 border">ชื่อ</th>
                                    <th class="text-center pa-2 border">นามสกุล</th>
                                    <th class="text-center pa-2 border">อีเมล</th>
                                    <th class="text-center pa-2 border">ชื่อผู้ใช้</th>
                                    <th class="text-center pa-2 border">ประเภทสมาชิก</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(items,index) in result " :key="items.id_member">
                                    <td class="text-center pa-2 border">{{ index+1 }}</td>
                                    <td class="text-center pa-2 border">{{ items.first_name }} {{ items.last_name }}</td>
                                    <td class="text-center pa-2 border">{{ items.email }}</td>
                                    <td class="text-center pa-2 border">{{ items.username }}</td>
                                    <td class="text-center pa-2 border">{{ items.role }}</td>
                                    <td class="text-center pa-2 border">
...
const del = async (id_member:number) => {
    try{
        ...
        await axios.delete(`${staff}/${id_member}`,{headers: {Authorization : `Bearer ${token}`}})
    }catch(err){
